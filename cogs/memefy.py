...
memes = dict()
try:
    response = request.Request("https://api.memegen.link/templates/", headers={'User-Agent': 'Mozilla/5.0'})
    meme_list = loads(request.urlopen(response).read())
    memes = {meme['name']: {'id': meme['id'], 'lines': meme['lines']}
...
class MemefyCommand(commands.Cog):
    ...
    async def meme(
        ...
        """
        req_url = f"{self.apiurl}/{self.memes[name]['id']}/{escape(top)}/{escape(bottom)}"
